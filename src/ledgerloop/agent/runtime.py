        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                if not exc.retryable:
                    # A malformed request or a bad credential will fail
                    # identically on every attempt - stop instead of burning
                    # the retry budget.
                    logger.warning("Runtime attempt %d failed unretryably: %s", attempt, exc)
                    break
                logger.warning(
                    "Runtime attempt %d/%d failed retryably: %s",
                    attempt,
                    self.max_retries,
                    last_error,
                )
